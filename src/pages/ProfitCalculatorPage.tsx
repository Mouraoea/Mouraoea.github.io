...
export function ProfitCalculatorPage() {
  ...
  const resetFilters = useCallback(() => {
    ...
    saveProfitFilterSettings(defaults);
  }, []);
  ...
  const priceMap = useMemo(
    () =>
      latestSnapshot && archive
        ? buildSanitizedPriceMap(latestSnapshot, archive.snapshots)
        : new Map(),
  ...
  const rows = useMemo((): RecipeRow[] => {
    if (!allRecipeFiles || !latestSnapshot) return [];
  ...
  return (
    <main className="page">
      ...
      {!loading && !error && allRecipeFiles && latestSnapshot && (
        <>
          {totalRecipeCount > 0 ? (
